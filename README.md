...
- `--reset-identity` — back up the worker's config directory (`/var/lib/gnosisvpn/.config/`, holding the HOPR
  identity, safe, and node database) by renaming it to `.config.<timestamp>.bak`, and remove the network override
...
- `GNOSISVPN_RESET_IDENTITY=true` — back up the worker's config directory (`/var/lib/gnosisvpn/.config/`, holding the
  HOPR identity, safe, and node database) by renaming it to `.config.<timestamp>.bak`, and remove the network
  override (`/etc/gnosisvpn/gnosisvpn-dynamic.env`), before the service starts, so a fresh identity is generated
  (default: `false`).
